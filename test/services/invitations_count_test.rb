...
class InvitationsCountTest < ActiveSupport::TestCase
  test "should return coaches invitations count" do
    ...
    Invitation.create!(client: client1, coach: coach1, status: 1) # Active client
    Invitation.create!(client: client2, coach: coach1, status: 1) # Active client
    Invitation.create!(client: client3, coach: coach1, status: 0)

    if CoachManager::InvitationsCount.call(coach1) == 1
      assert true
    else
      assert false
    end
